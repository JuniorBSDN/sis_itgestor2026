...
# --- FUNÇÃO CORRIGIDA: GERAR CERTIFICADOS ---
def gerar_pdf_bytes(nome, cpf, curso):
    ...
    pdf.ln(10)
    pdf.multi_cell(0, 8, "INSTITUTO IMPAR & HOSPITAL MUNICIPAL DE VIGIA DE NAZARE - RAIMUNDO VASCONCELOS", align='C')
...
def enviar_email(destinatario, nome, pdf_buffer):
    try:
        ...
        msg.attach(MIMEText("Segue em anexo o seu certificado de conclusao.", 'plain'))
...
def registrar():
    try:
        ...
        if not dados or 'nome' not in dados: return jsonify({"status": "erro"}), 400
        dados['data_conclusao'] = datetime.now(TZ_PA).strftime('%d/%m/%Y %H:%M')
        if 'cpf' in dados: dados['cpf'] = "".join(filter(str.isdigit, str(dados['cpf'])))
        db.collection('treinamentos').add(dados)
        if dados.get('acao') == 'email' and dados.get('email'):
            # PEGA O CURSO DO JSON
            curso_nome = dados.get('curso', 'Treinamento HMV')
            pdf_buf = gerar_pdf_bytes(dados['nome'], dados.get('cpf', '000'), curso_nome)
...
def download():
    try:
        ...
        # PEGA O CURSO DA URL
        curso_nome = request.args.get('curso', 'Treinamento HMV')
        pdf_buf = gerar_pdf_bytes(nome, cpf, curso_nome)
        pdf_buf.seek(0)
